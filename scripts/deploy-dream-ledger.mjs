...
const source = await readFile(resolve(process.cwd(), "contracts/DreamLedger.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "DreamLedger.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["DreamLedger.sol"]?.DreamLedger;
...
console.log(`Deploying DreamLedger from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_DREAM_LEDGER_CONTRACT_ADDRESS to this contract address in Vercel.");
